...
from model.modules_ACD import MLP, SelfAtt, GNN
...
class GNNENC(GNN):
    ...
    def forward(self, x: Tensor, es: Tensor) -> Tensor:
        ...
        """
        x = x.permute(1, 0, 2, -1).contiguous()
...
class RNNENC(GNN):
    ...
    """
    def __init__(self, n_in: int, n_hid: int, n_edge_types: int, n_states: int,
                 do_prob: float=0., factor: bool=True,
        ...
        if option == 'both':
            self.fc_out = nn.Linear(n_hid * 2, n_edge_types*n_states)
        else:
    ...
    def forward(self, x, es):
        ...
        if self.option == 'both':
            ...
            z = z.view(-1, batch, dim)
        z = self.fc_out(z).transpose(0,1).view(batch, E, self.n_states, self.n_edge_types)
        return z
